<template>
  <div class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
      ...
      <form @submit.prevent="saveVendor" class="p-6 space-y-4">
        <!-- Error Message -->
...
const saveVendor = async () => {
  ...
  try {
    if (props.vendor) {
      await vendorsApi.updateVendor(props.vendor.id, form.value)
      success('Vendor updated successfully')
    } else {
      await vendorsApi.createVendor(form.value)
    ...
    errorMessage.value = err.response?.data?.error || 'Error saving vendor'
